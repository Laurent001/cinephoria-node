...
import { fetchQRCodeByBookingId } from "../controllers/qrcodeController.ts";
...
const getSeatsByScreeningId = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const screening: Screening = {
      ...
      auditorium_name: rows[0].auditorium_name,
      auditorium_seat: rows[0].auditorium_seat,
      auditorium_seat_handi: rows[0].auditorium_seat_handi,
    ...
    const seats: Seat[] = rows.map((row: any) => ({
      id: row.seat_id,
